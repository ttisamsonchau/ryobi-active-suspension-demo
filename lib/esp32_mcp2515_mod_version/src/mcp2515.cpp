...
MCP2515::MCP2515(spi_device_handle_t *s, uint8_t MISO, uint8_t MOSI, uint8_t SCK, uint8_t CS, int clk_spd)
{
    spi = s;
    // ESP32SIM800 START
    esp_err_t ret;
    ...
    spi_device_interface_config_t devcfg = {};
        devcfg.clock_speed_hz = clk_spd, // Clock out at 1 MHz
        devcfg.mode = 0,
...
MCP2515::ERROR MCP2515::sendMessage(const TXBn txbn, const struct can_frame *frame, bool ext)
{
...
MCP2515::ERROR MCP2515::sendMessage(const struct can_frame *frame, bool ext)
{
    if (frame->can_dlc > CAN_MAX_DLEN)
    {
        return ERROR_FAILTX;
    ...
    for (int i = 0; i < N_TXBUFFERS; i++)
    {
        ...
        {
            return sendMessage(txBuffers[i], frame, ext);
        }
    }
