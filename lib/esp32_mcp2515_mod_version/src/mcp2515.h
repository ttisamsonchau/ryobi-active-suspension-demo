...
class MCP2515
{
    ...
    public:
        MCP2515(spi_device_handle_t *s, uint8_t MISO, uint8_t MOSI, uint8_t SCK, uint8_t CS, int clk_spd);
        ERROR reset(void);
        ERROR setConfigMode();
        ...
        ERROR sendMessage(const TXBn txbn, const struct can_frame *frame, bool ext);
        ERROR sendMessage(const struct can_frame *frame, bool ext);
